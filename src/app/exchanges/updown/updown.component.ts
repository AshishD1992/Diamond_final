...
export class UpdownComponent implements OnInit {
  ...
  modalRef!: BsModalRef;
   ...
   ngOnInit(): void {
    ...
    this.casinoService.connectCasino("http://45.76.155.250:11005/", this.gameType);
    this.getCasinoData();
  ...
  getCasinoData() {
    ...
    this.casinoSubscription = this.casinoService.casinoSource.subscribe(data => {
      console.log(data,'casino lucky7');
      if (data != null) {
  ...
  GetRecentGameResult() {
    this.reportService.GetRecentGameResult(this.gameType).subscribe((resp:any)=>{
      _.forEach(resp.data,(element) => {
        if(element.result.indexOf('HIGH Card')>-1){
          element['card']="H";
          element['player']='b';
        }else if(element.result.indexOf('LOW Card')>-1){
          element['card']="L";
          element['player']='a';
        }else if(element.result.indexOf('TIE Card')>-1){
          element['card']="T";
          element['player']='c';
        }
      });
  ...
  PlaceTpBet() {
    ...
    this.betService['PlaceTpBet'](this.OpenBetForm.value).subscribe(resp => {
  ...
  Lucky7ExposureBook() {
    this.betService.Lucky7ExposureBook(this.gameId).subscribe(resp => {
      this.pnl = resp.data;
    }, err => {
      if (err.status == 401) {
        this.toastr.error(err.error.description.result);
